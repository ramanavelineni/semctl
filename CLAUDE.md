...
- CWD configs (`./semctl.yaml`, `./.semctl.yaml`) load ONLY when trusted (direnv-style, `internal/config/trust.go`): trust = abs path + content sha256 in `~/.config/semctl/trusted.yaml`, lapses on any content edit. Consent = TTY prompt (`config.TrustPrompt`, wired in `Execute()`) or `semctl trust` (`--revoke`/`--list`); explicit `--config` bypasses the gate. Untrusted → skipped, home config used, warning via `warnIfUntrustedSkipped()` (root PersistentPreRunE + the self-loading commands: login/logout/context). Load parses the exact bytes that were hashed (no TOCTOU)
...
- Commands skipped in PersistentPreRunE: login, logout, completion, version, __complete, trust (session flags like --timeout are processed before the skip)
